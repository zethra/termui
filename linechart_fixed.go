...
func NewFixedLineChart() *FixedLineChart {
	lc := &FixedLineChart{Block: *NewBlock()}
...
func (lc *FixedLineChart) renderBraille() Buffer {
	buf := NewBuffer()
...
func (lc *FixedLineChart) renderDot() Buffer {
	buf := NewBuffer()
...
func (lc *FixedLineChart) calcLabelX() {
	lc.labelX = [][]rune{}
...
func shortenFloatValFixed(x float64) string {
	s := fmt.Sprintf("%.2f", x)
	if len(s)-3 > 3 {
		s = fmt.Sprintf("%.2e", x)
	}
...
func (lc *FixedLineChart) calcLabelY() {
	...
	maxLen := 0
	for i := 0; i < n; i++ {
		s := str2runes(shortenFloatValFixed(lc.BottomValue + float64(i)*span/float64(n)))
		if len(s) > maxLen {
			maxLen = len(s)
		}
		lc.labelY[i] = s
...
func (lc *FixedLineChart) calcLayout() {
...
func (lc *FixedLineChart) plotAxes() Buffer {
	buf := NewBuffer()
...
func (lc *FixedLineChart) Buffer() Buffer {
	buf := lc.Block.Buffer()
